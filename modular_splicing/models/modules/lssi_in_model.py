...
class BothLSSIModels(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        no_motif_transform=False,
    ):
        super().__init__()
        self.use_splicepoint = None if no_linear_layers else nn.Linear(2, 3)
        self.splicepoint_transform_motif = self._construct_motif_transformer(
            no_linear_layers=no_linear_layers,
            no_motif_transform=no_motif_transform,
        )
    ...
    @classmethod
    def _construct_motif_transformer(cls, *, no_linear_layers, no_motif_transform):
        if no_linear_layers:
            assert not no_motif_transform
            return None
        if no_motif_transform:
            return nn.Identity()
